	...
	for(i=0;i <10;i++)
	{
		for(j = 0;j < 10;j++)
		{
			if((i+j) % 2==0)
				...
				setWindow(0.0,screenWidth,screenHeight,0.0);
			glViewport( i*64, j*44, 64, 44 ) ; 
			drawPolylineFile( "dino.dat" ) ;
		}
	}
	glFlush();
	SOIL_save_screenshot("/home/xpd54/personal/mywork/cg/output/muti_flip_dino.bmp",SOIL_SAVE_TYPE_BMP,0,0,screenWidth,screenHeight);
}
